...
public class PolygonServiceImpl implements PolygonService {
	...
	@Override
	public void create(Polygon poligono) {
					
		String xpoint=poligono.getXPoints().replaceAll("\\s", "");
		String ypoint=poligono.getYPoints().replaceAll("\\s", "");
		...
		if(numpointX==numpointY) {			
			if(numpointX==poligono.getNpoints()) {	
				poligono.setXPoints(Arrays.toString(xpointArray));
				poligono.setYPoints(Arrays.toString(ypointArray));
				poligono = polygonRepo.save(poligono);	
			}
		}	
	}
	...
	private void convertStringInIntegerArray(String xPoints, String yPoints, Integer[] intArrayX, Integer[] intArrayY) {
		...
		String cleanedXPoints = xPoints.substring(1, xPoints.length() - 1).trim();
		String cleanedYPoints = yPoints.substring(1, yPoints.length() - 1).trim();
